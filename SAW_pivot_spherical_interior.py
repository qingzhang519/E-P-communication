# -*- coding: utf-8 -*-
"""
# This is the python code to simulate a SAW chain walking inside a sphere using the pivot algorithm.
"""
import numpy as np
...
for h in range(1, 101):
    ...
    if np.max(np.sum(Walk[:, :, 0]**2, axis=1)) < R**2:
        break
...
for i in range(1, N_run):
    ...
    if s == 1:
        ...
        if np.max(np.sum(Xd**2, axis=1)) <= R**2:
            Walk[:, :, i] = Xd
        continue
    ...
    if s < int(N_step/2):
        ...
        if flag == 0 or np.max(np.sum(Walk[:s-1, :, i]**2, axis=1)) > R**2:
